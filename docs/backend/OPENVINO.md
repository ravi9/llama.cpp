...
| `GGML_OPENVINO_DISABLE_KV_STATE_RELAYOUT` | Boolean | `0`     | Disable the stateful KV-state sequence-axis relayout (relayout is on by default). It moves the KV state sequence axis from dim 1 to dim 2 for models with a single KV head, so the GPU plugin can append new tokens in place instead of copying the whole state every token. Set to `1` to disable. |
