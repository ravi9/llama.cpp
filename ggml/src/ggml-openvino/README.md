...
- Blobs contain weights and can approach model size for each compiled graph. Import still reads those blobs and initializes the device. Existing exact-graph entries remain usable. Old cache directories may contain a `settings` file; it is no longer read or written.
