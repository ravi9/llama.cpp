...
static bool is_op_unsupported_case(const ggml_tensor * op) {
    ...
    case GGML_OP_MUL_MAT: {
        ...
        if ((op->src[0]->op == GGML_OP_PERMUTE || op->src[1]->op == GGML_OP_PERMUTE) &&
            op->src[1]->src[0]->op != GGML_OP_VIEW) {
            return true;
        }
    ...
    case GGML_OP_CONT: {
        if (op->src[0] == nullptr || (op->src[0]->op != GGML_OP_PERMUTE && !(op->src[0]->op == GGML_OP_TRANSPOSE && op->src[0]->src[0] != nullptr && op->src[0]->src[0]->op == GGML_OP_PERMUTE))) {
            return true;
        }
        break;
    }
